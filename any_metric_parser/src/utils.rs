...
fn _parse_regex(filename: &str) -> (String, Option<i32>, String){
    let properties = match RE.captures(&filename){
        Some(res) => res,
        None => {
            panic!("{}", filename);
        }
        ...
        if target.file_type()?.is_dir(){
            ...
            for summary_file in glob(curdir.join("newdefault*.summary.gz").to_str().unwrap()).expect("Glob failed"){
                ...
                pool_outer.execute(move || {
                    parse_target(&target_name, input_dir, output_dir, pool_inner, &summary_file, &compare_type).unwrap();
                });

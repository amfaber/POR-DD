use std::{path::PathBuf, fs, io::{BufRead, Write, BufReader, Read, ErrorKind}, sync::mpsc};
use threadpool::ThreadPool;
use polars::prelude::*;
use std::collections::{HashMap, HashSet};

pub fn parse_df<P: Into<PathBuf>>(
    input_dir: P,
    output_dir: P,
    df_path: &str,
    ){
    ...
    let mut already_created_files = HashSet::new();
    let df = lf.
    select(
    ...
    for directory_ligands_df in directory_ligands_dfs{
        ...
        let name = ligands.splitn(3, "_").nth(1).unwrap();
        // name.push_str(".sdf");
        let mut write_path = output_dir.join(target).join(name);
        write_path.set_extension("sdf");

        dbg!(&write_path);
        match fs::create_dir(write_path.parent().unwrap()){
            Ok(_) => {},
            Err(e) => {
                match e.kind(){
        ...
        let mut writer: fs::File;
        match already_created_files.insert(write_path.to_str().unwrap().to_string()){
            true => {
                writer = fs::OpenOptions::new().write(true).create(true).open(write_path).unwrap();
            },
            false => {
                writer = fs::OpenOptions::new().write(true).append(true).open(write_path).unwrap();
            }
        }
        writer.write_all(full_mols.as_bytes()).unwrap();
    }
